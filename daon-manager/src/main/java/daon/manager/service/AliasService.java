...
public class AliasService {
    ...
    public List<Index> list() throws ExecutionException, InterruptedException {

        GetIndexResponse response = client.admin().indices().getIndex(new GetIndexRequest().features(GetIndexRequest.Feature.ALIASES)).get();

        ImmutableOpenMap<String, List<AliasMetaData>> aliases = response.getAliases();
        List<Index> list = Arrays.stream(response.getIndices()).map(name->{

            Index index = new Index();
            index.setName(name);

            List<AliasMetaData> aliasMetaData = aliases.get(name);

            if(aliasMetaData != null) {
                List<String> alias = aliasMetaData.stream().map(AliasMetaData::alias).sorted().collect(Collectors.toList());
...
//    }
    ...
    public boolean save(AliasIndices data) {

        boolean isSuccess = true;

        String alias = data.getAlias();

        String[] removeIndexNames = data.getOptions().toArray(new String[0]);
        String[] addIndexNames = data.getIndices().toArray(new String[0]);
