...
public class TransportModelReloadAction extends TransportMasterNodeReadAction<ModelReloadRequest, ModelReloadResponse> {

    private final GatewayAllocator gatewayAllocator;

    @Inject
    public TransportModelReloadAction(Settings settings, TransportService transportService, ClusterService clusterService,
                                      ThreadPool threadPool, ActionFilters actionFilters,
                                      IndexNameExpressionResolver indexNameExpressionResolver, GatewayAllocator gatewayAllocator) {
        super(settings, ModelReloadAction.NAME, false, transportService, clusterService, threadPool, actionFilters,
            indexNameExpressionResolver, ModelReloadRequest::new);
    ...
    @Override
    protected ClusterBlockException checkBlock(ModelReloadRequest request, ClusterState state) {
    ...
    @Override
    protected ModelReloadResponse newResponse() {
        return new ModelReloadResponse();
    }

    @Override
    protected final void masterOperation(ModelReloadRequest request, ClusterState state, ActionListener<ModelReloadResponse> listener) throws Exception {
        logger.warn("attempt to execute a cluster health operation without a task");
        throw new UnsupportedOperationException("task parameter is required for this operation");
    }

    @Override
    protected void masterOperation(Task task, final ModelReloadRequest request, final ClusterState unusedState, final ActionListener<ModelReloadResponse> listener) {
    ...
    private void executeHealth(final ModelReloadRequest request, final ActionListener<ModelReloadResponse> listener) {
        int waitFor = 5;
        ...
        final ClusterStateObserver.Listener stateListener = new ClusterStateObserver.Listener() {
            ...
            public void onTimeout(TimeValue timeout) {
                final ModelReloadResponse response = getResponse(request, observer.setAndGetObservedState(), concreteWaitFor, true);
    ...
    private boolean validateRequest(final ModelReloadRequest request, ClusterState clusterState, final int waitFor) {
        ModelReloadResponse response = clusterHealth(request, clusterState, clusterService.numberOfPendingTasks(),
    ...
    private ModelReloadResponse getResponse(final ModelReloadRequest request, ClusterState clusterState, final int waitFor, boolean timedOut) {
        ModelReloadResponse response = clusterHealth(request, clusterState, clusterService.numberOfPendingTasks(),
                gatewayAllocator.getNumberOfInFlightFetch(), clusterService.getMaxTaskWaitTime());
    ...
    private boolean prepareResponse(final ModelReloadRequest request, final ModelReloadResponse response, ClusterState clusterState, final int waitFor) {
    ...
    private ModelReloadResponse clusterHealth(ModelReloadRequest request, ClusterState clusterState, int numberOfPendingTasks, int numberOfInFlightFetch,
        ...
        try {
            ...
            ModelReloadResponse response = new ModelReloadResponse(clusterState.getClusterName().value(), Strings.EMPTY_ARRAY, clusterState,
        ...
        return new ModelReloadResponse(clusterState.getClusterName().value(), concreteIndices, clusterState, numberOfPendingTasks,
